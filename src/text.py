...
class text_mining(object):
    ...
    def filter_sim_matrix(self, matrix, columns, n_sim=100, percentil="50%", value_min=0, value_max=0.99, colum_value='value'):
        ...
        for num_line, irow in enumerate(matrix.iterrows()):
            ...
            for num_col, j in enumerate(row.index):
                ...
                logic_filter = value>=value_min and value<=value_max #
        ...
        df_maxtrix_filter = df_maxtrix_filter.drop_duplicates(subset=["doc_a","doc_b"])
        del list_filter
