...
class grobid_cli(object):
    ...
    def process_pdf(self, client, pdf_file, service="processFulltextDocument", generateIDs=True, include_raw_citations=True,
                    include_raw_affiliations=True, consolidate_header=False, consolidate_citations=False, tei_coordinates=False,
                    segment_sentences=True):
        
        """"""
        
        pdf_file, status, xml = client.process_pdf(service=service,
                                                   pdf_file=pdf_file,
    ...
    def process_pdfs(self, client, input_path, n_workers, service="processFulltextDocument", generateIDs=True, include_raw_citations=True,
                    include_raw_affiliations=True, consolidate_header=False, consolidate_citations=False, tei_coordinates=False,
                    segment_sentences=True,verbose=True):
        
        """"""
        
        list_results = client.batch_process(service=service,
...
class xmltei_to_dataframe(object):
    ...
    def get_dataframes_article(self,doc):
        ...
        dict_dataframes = {'df_doc_info':df_doc_info,
                           'df_doc_head':df_doc_head,
                           'df_doc_authors':df_doc_authors,
                           'df_doc_citations':df_doc_citations,
                           'df_doc_authors_citations':df_doc_authors_citations}
